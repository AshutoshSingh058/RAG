...
with tab3:
    ...
    if not st.session_state.pipeline_done:
        ...
        st.info(
            "Eval traffic is tagged with Portkey feature `eval`. "
            "Each sample is processed individually to control token usage.",
            icon="ℹ️",
        )

        run_metrics_col, smoke_test_col = st.columns(2)
        run_metrics_btn = run_metrics_col.button(
            "▶️ Run Eval Metrics",
            type="primary",
            disabled=not st.session_state.pipeline_done,
        )
        smoke_test_btn = smoke_test_col.button(
            "🧪 Test 1 Sample",
            disabled=not st.session_state.pipeline_done,
        )

        if run_metrics_btn or smoke_test_btn:
            status_slot = st.empty()
            results_slots = {}
            ...
            with logfire.span("📊 Streamlit — Run Metrics Button"):
                metric_results = _run_async(
                    run_all_metrics(
                        ...
                        status_cb=status_cb,
                        sample_limit=1 if smoke_test_btn else None,
                    )
                )
